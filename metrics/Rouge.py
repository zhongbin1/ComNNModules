...
def _lcs(x, y):
    """Computes the length of the longest common subsequence (lcs) between two strings. The implementation below uses a
    DP programming algorithm and runs in O(nm) time where n = len(x) and m = len(y).
    Source: http://www.algorithmist.com/index.php/Longest_Common_Subsequence
    Args:
...
def _recon_lcs(x, y):
    ...
    def _recon(i_, j_):
        ...
        if i_ == 0 or j_ == 0:
            ...
            return _recon(i_ - 1, j_ - 1) + [(x[i_ - 1], i_)]
        elif table[i_ - 1, j_] > table[i_, j_ - 1]:
            return _recon(i_ - 1, j_)
        else:
            return _recon(i_, j_ - 1)

    recon_tuple = tuple(map(lambda x_: x_[0], _recon(i, j)))
    return recon_tuple
...
def _union_lcs(evaluated_sentences, reference_sentence):
    """
    Returns LCS_u(r_i, C) which is the LCS score of the union longest common subsequence between reference sentence ri
    and candidate summary C. For example if r_i= w1 w2 w3 w4 w5, and C contains two sentences: c1 = w1 w2 w6 w7 w8 and
    c2 = w1 w3 w8 w9 w5, then the longest common subsequence of r_i and c1 is "w1 w2" and the longest common subsequence
    of r_i and c2 is "w1 w3 w5". The union longest common subsequence of r_i, c1, and c2 is "w1 w2 w3 w5" and
    LCS_u(r_i, C) = 4/5.
    Args:
        evaluated_sentences: The sentences that have been picked by the summarizer
...
def rouge(hypotheses, references):
    ...
    cf. https://github.com/tensorflow/nmt/blob/master/nmt/scripts/rouge.py
    """
